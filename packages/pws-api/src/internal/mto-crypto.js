import { decryptWithAesGcm, decryptWithAesGcmPadding, decryptWithPasswordProviderV2, decryptWithPasswordProviderV3, encryptWithAesGcm, encryptWithAesGcmPadding, encryptWithPasswordProviderV2, encryptWithPasswordProviderV3, generateAesKey, pbkdf2, } from './crypto/aes.js';
import { decryptWithEcc, encryptWithEcc, generateEccKey, signWithEccPrivateKey, } from './crypto/ecc.js';
import { decryptWithPasswordProviderV1, decryptWithRsaProvider, encryptWithPasswordProviderV1, encryptWithRsaProvider, getPrivateKeyType, getPublicKeyType, NativeRsa, } from './crypto/rsa.js';
import { MTO_ENCRYPTION_CHAIN, MTO_HASH_ALGORITHM, binaryToUtf8, concatBytes, createClientInformation, getClientVersion, randomUint8Array, toBinaryString, toUint8Array, utf8ToBinary, usesSymmetricKey, } from './crypto/shared.js';
export async function calculateMtoHash(hashAlgorithm, password, saltBase64) {
    if (password == null) {
        return {
...
export async function generateRsaPkcs1Key() {
...
export async function encryptWithDetectedPublicKey(publicKey, value) {
    return getPublicKeyType(publicKey) === 'RSA'
        ? encryptWithRsaProvider(publicKey, value)
        : encryptWithEcc(publicKey, value);
}
export async function signWithDetectedPrivateKey(privateKey, value) {
...
export async function mtoEncrypt(chain, key, value) {
    if (value == null) {
        return '';
    }
    let encryptedValue = '';
...
export async function mtoDecrypt(key, encryptedValue) {
    if (!encryptedValue) {
        return '';
    }
...
export function encodeUtf8Binary(value) {
