import { concatBytes, deserializeCurve521Parameter, serializeEccParameter, toSerializedKeyBytes, toUint8Array, } from './shared.js';
...
function encodeBinHeader(length) {
    if (length < 0x100) {
    ...
    return Uint8Array.of(0xc6, (length >> 24) & 0xff, (length >> 16) & 0xff, (length >> 8) & 0xff, length & 0xff);
}
function encodeMessagePackBinaryArray(values) {
    return concatBytes(encodeArrayHeader(values.length), ...values.flatMap((value) => [encodeBinHeader(value.length), value]));
}
function readLength(bytes, offset, prefix) {
    if (prefix <= 0xbf) {
...
function decodeMessagePackBinaryArray(value) {
    let offset = 0;
...
export function serializeEccPrivateKey(privateKey) {
    return encodeMessagePackBinaryArray([
...
export function deserializeEccPrivateKey(value) {
...
export function serializeAsymmetricEccRightKey(value) {
    return encodeMessagePackBinaryArray([
        value.eccPublicKey,
        value.salt,
        value.aesIv,
...
export function deserializeAsymmetricEccRightKey(value) {
    const [eccPublicKey, salt, aesIv, encryptedValue] = decodeMessagePackBinaryArray(toUint8Array(value));
