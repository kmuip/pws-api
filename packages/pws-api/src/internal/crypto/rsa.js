...
import { RSA_SEPARATOR, toBinaryString, toSerializedKeyBytes, utf8ToBinary, } from './shared.js';
function parseXmlTag(tagName, xml) {
...
function decodeMaybeBase64Xml(value) {
    let decoded = value;
    let attempts = 0;
...
export class NativeRsa {
    ...
    publicKeyFromXml(value) {
        const xml = decodeMaybeBase64Xml(value);
        this.publicKey = forge.pki.setRsaPublicKey(bigIntegerFromBase64(parseXmlTag('Modulus', xml)), bigIntegerFromBase64(parseXmlTag('Exponent', xml)));
    }
    privateKeyFromXml(value) {
        const xml = decodeMaybeBase64Xml(value);
        this.privateKey = forge.pki.setRsaPrivateKey(bigIntegerFromBase64(parseXmlTag('Modulus', xml)), bigIntegerFromBase64(parseXmlTag('Exponent', xml)), bigIntegerFromBase64(parseXmlTag('D', xml)), bigIntegerFromBase64(parseXmlTag('P', xml)), bigIntegerFromBase64(parseXmlTag('Q', xml)), bigIntegerFromBase64(parseXmlTag('DP', xml)), bigIntegerFromBase64(parseXmlTag('DQ', xml)), bigIntegerFromBase64(parseXmlTag('InverseQ', xml)));
    }
    publicKeyToXml() {
        if (!this.publicKey) {
...
export function getPublicKeyType(value) {
...
export function getPrivateKeyType(value) {
    const bytes = toSerializedKeyBytes(value);
    if (bytes[0] === 80 && bytes[1] === 70) {
        return 'RSA';
    }
...
export async function encryptWithPasswordProviderV1(password, plaintext) {
...
export async function decryptWithPasswordProviderV1(password, encryptedValue) {
...
export async function encryptWithRsaProvider(publicKey, plaintext) {
...
export async function decryptWithRsaProvider(privateKey, encryptedValue) {
    const parts = toBinaryString(encryptedValue).split(RSA_SEPARATOR);
    if (parts.length < 5) {
        return '';
    }
