...
function characterEntropy(values: number[]) {
  if (values.length === 0) {
    return 0
  }
...
function separatorValue(separator: number) {
  switch (separator) {
    case 3:
      return '-'
    case 2:
...
export class RuntimePasswordManager implements PasswordManager {
  constructor(private readonly errors: PasswordPolicyErrorCodes) {}
  ...
  validatePassword(
    ...
    if (
      policy.NotAllowedPasswordUsername &&
  ...
  generatePolicyPassword(policy: PsrPolicy, usernames: string[] = []) {
    if (!policy) {
      return ''
    }
